...
void funcion_filosofos( int id )
{
  ...
  while ( true )
  {
    //Sentarse en la mesa (camarero decide) - 5 filosofos kk - 4 OK 
    //Si no interbloqueo -> orden coger tenedores
    ...
    //Piden levantarse de la mesa al camarero -> solo peticiones de levantarse si 4 comen -> tag
...
void funcion_camarero()
{
  ...
  while(true)
  {
    //1ro, seleccionar etiqueta aceptable
    ...
    //Realizar accion en funcion del tag usado

    switch( estado.MPI_TAG ) // leer emisor de la eti1ueta en metadatos
      {
        case tag_levantarse: 
          filosofos_sentados--;
          cout << "  Se levanta el filosofo: " << estado.MPI_SOURCE << endl;
        break;

        case tag_sentarse: 
          filosofos_sentados++;
          cout << "  Se sienta el filosofo: " << estado.MPI_SOURCE << endl;
        break;
      }
